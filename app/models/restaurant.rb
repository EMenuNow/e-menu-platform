...
class Restaurant < ApplicationRecord
  ...
  def available_times(offset = 0)
    ...
    delivery_time_options << {value: "ASAP", text: "ASAP"} if is_open and (round_down_t + dtm < closing_time) and !opening_time_close_early and busy_times.where(busy_time: busy_now).empty?
    ...
    if offset == 0 and !opening_time_close_early
      until rounded_t > time_closing - btm
        if rounded_t >= next_time
          ...
          delivery_time_options << {value: rounded_t.strftime("%H:%M"), text: "#{rounded_t.strftime("%H:%M")}", busy: busy_times.where(busy_time: is_busy).any?}
        end
      ...
      until next_time > time_closing
        ...
        delivery_time_options << {value: next_time.strftime("%H:%M"), text: "#{next_time.strftime("%H:%M")}", busy: busy_times.where(busy_time: is_busy).any?} 
